...
static __volatile struct limine_memmap_request memmap_request = {
...
static void spin_lock(SPIN_LOCK* lock) {
    while (__sync_lock_test_and_set(&lock->locked, 1)) {
        while (lock->locked) __asm__ volatile("pause");
    }
}

static void spin_unlock(SPIN_LOCK* lock) {
    __sync_lock_release(&lock->locked);
}

static void* allocate_pages(size_t count) {
    size_t size = count * PAGE_SIZE;
    if (heap_current + size > heap_start + heap_size) return NULL;
...
static SLAB* create_slab(CACHE* cache) {
    ...
    SLAB* slab = (SLAB*)allocate_pages(1);
    if (!slab) return NULL;
...
CACHE* cache_create(size_t size, int align, int flags) {
    CACHE* cache = (CACHE*)allocate_pages(1);
